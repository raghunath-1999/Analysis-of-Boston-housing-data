...
Analyzing following mentioned plot before and after removing outliers, plots are residuals vs fitted, residuals boxplot and qq plot
...
cat("Influentials observations with respective to LSTAT=",length(outliers_wrt_LSTAT),"\n")
```
#####To be completed Aussie
```{r}
# library(dplyr)
# # dfbetas[(abs(dfbetas$LSTAT) > thresh | abs(dfbetas$LSTAT) < thresh) | (abs(dfbetas$B) > thresh | abs(dfbetas$B) < thresh),]
# 
# # head(dfbetas)
# # dfbetas <- filter(dfbetas,LSTAT>thresh)
# 
# nrows(dfbetas)
# dfbetas %>% filter_all(any_vars(.> thresh | .< thresh))
# 
# # which(dfbetas$LSTAT >thresh)
# length(which(dfbetas$LSTAT <thresh |))
# 
# print(length(dfbetas$LSTAT))
```
...
regression for each quantitative variabl
```{r}
